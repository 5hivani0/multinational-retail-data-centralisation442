    ...
    def convert_product_weights(self):
        ...
        for weight in self.df['weight']:
            if weight[-2:] == "kg":
                ...
                converted_weights_in_kg.append(int(weight.replace('kg', '')))
            elif weight[-1:] == "g":
                # Remove 'g', change to float, and divide by 1000
                converted_weights_in_kg.append(int(weight.replace('g', '')) / 1000)
            elif weight[-2:] == "ml":
                # Remove 'ml', change to float, and divide by 1000
                converted_weights_in_kg.append(int(weight.replace('ml', '')) / 1000)
            else:
